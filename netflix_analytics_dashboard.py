...
GCP_PROJECT = os.environ.get("GCP_PROJECT", "ntu-dsai-t4-netflix")
...
3. Set env vars:     `export GCP_PROJECT=ntu-dsai-t4-netflix`
    ...
    fact = bq(f"""
        SELECT
            ...
            PARSE_DATE('%B %d, %Y', date_added)                 AS date_added,
            ...
            EXTRACT(MONTH FROM PARSE_DATE('%B %d, %Y', date_added)) AS month_added
    ...
    dim_ttl = bq(f"SELECT show_id, title, director, cast FROM {ds}.dim_showtitle")
